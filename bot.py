...
BOT_USERNAME = os.getenv("BOT_USERNAME", "osintJit1_bot")  # ← исправлено
...
def format_short(data, query, stats_count, search_type):
    ...
    if search_type == "phone":
        ...
        if phone_info:
            ...
            # Если DaData вернул данные, выводим их
            if operator or region or country:
                if operator:
                ...
                # Если DaData вернул None, пробуем BigBase
                ...
                if bigbase_data and isinstance(bigbase_data, dict):
                    ...
                    cou = head.get("phone_country_info")
                    if op:
                        lines.append(f"├ Оператор: {op}")
                    if reg:
                        lines.append(f"├ Регион: {reg}")
                    if cou:
                        lines.append(f"├ Страна: {cou}")
        else:
            # Если phone_info вообще нет, пробуем BigBase напрямую
            bigbase_data = data.get("bigbase")
            if bigbase_data and isinstance(bigbase_data, dict):
                dossier = bigbase_data.get("dossier", {})
                head = dossier.get("head", {})
...
async def start(update, context):
    ...
    # Логирование для отладки реферальной ссылки
    logging.info(f"START args: {context.args}")
    
    # Проверка реферальной ссылки в формате ref_123456789
    if context.args:
        arg = context.args[0]
        if arg.startswith("ref_"):
            ...
            if referrer_id.isdigit():
                if add_refferal(user_id, referrer_id):
                    ...
                    await update.message.reply_text(
                        "Вы уже зарегистрированы или ссылка недействительна."
                    )
