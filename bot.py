...
async def handle_message(update, context):
    ...
    # Если поиск по ФИО – сразу отправляем HTML
    if search_type == 'name':
        ...
        try:
            with open(tmp_path, 'rb') as f:
                await update.message.reply_document(
                    document=f,
                    filename=f"infoHunt_report_name.html",
                    caption=f"📊 Отчёт по ФИО: {text}"
                )
        finally:
            os.unlink(tmp_path)
        return

    # Если поиск по телефону – краткий отчёт + кнопка
