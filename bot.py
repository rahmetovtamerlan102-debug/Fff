...
# ========== ГЕНЕРАЦИЯ HTML (без поля "source", без смайликов) ==========
def generate_html(data, search_type, query):
    ...
    # Формируем список источников (без поля source)
    sources = []
    ...
    # Карточки баз – выводим все поля, кроме "source"
    cards_html = ""
    for i, rec in enumerate(results[:20], 1):
        # Удаляем поле "source" из отображения
        filtered_rec = {k: v for k, v in rec.items() if k.lower() != "source"}
        # Формируем строку с парами ключ-значение в виде таблицы
        rows = "".join(
            f'<div class="row"><span class="key">{k}</span><span class="val">{v}</span></div>'
            for k, v in filtered_rec.items() if v is not None and v != ""
        )
        if not rows:
            rows = '<div class="row"><span class="key">Данные</span><span class="val">пусто</span></div>'
        
        size_kb = max(1, len(str(filtered_rec)) // 1024)
        cards_html += f'''
        <div class="card">
            <div class="card-head">
