...
def get_main_keyboard():
    buttons = [
        ...
        [InlineKeyboardButton("Партнерская программа", callback_data="refferal")],
    ]
...
def get_partial_search_keyboard():
    buttons = [
        ...
        [InlineKeyboardButton("Дата рождения", callback_data="partial_bdate")],
        [InlineKeyboardButton("Возраст (от-до)", callback_data="partial_age")],
        [InlineKeyboardButton("Место рождения", callback_data="partial_birthplace")],
        [InlineKeyboardButton("Готово -> Искать", callback_data="partial_search")],
...
def format_short(data, query, stats_count, search_type):
    ...
    leakosint_count = format_leakosint_count(data.get("leakosint"))
    if leakosint_count:
    ...
    for err in data.get("errors", []):
        if isinstance(err, dict):
            continue
        err_str = str(err)
        if "leakosint" in err_str.lower() and "ошибка" in err_str.lower():
            continue
        if "seon" in err_str.lower() and "{}" in err_str:
            continue
...
async def start(update, context):
    ...
    await update.message.reply_text(
        ...
        "Я сам определю, что это: телефон, ФИО, email, VK, Telegram и т.д.\n\n"
        "Примеры:\n"
        " 79271234567\n"
        ...
        " vk.com/id123\n"
        " tg123456789\n\n"
        "Или нажмите кнопку для поиска по неполным данным:",
...
async def partial_search_callback(update, context):
    ...
    await query.edit_message_text(
        ...
        "Нажмите на поле, которое хотите заполнить, а затем отправьте значение:",
...
async def partial_field_callback(update, context):
    ...
    field_names = {
        ...
        'firstname': 'Имя',
        'middlename': 'Отчество',
        'bdate': 'Дату рождения (ДД.ММ.ГГГГ или год)',
        'age': 'Возраст (от-до, например: 25-30)',
...
async def partial_search_execute(update, context):
    ...
    for field, values in partial.items():
        if values:
            if field == 'bdate':
                ...
                age_parts = values[0].split('-')
                if len(age_parts) == 2:
                    query_parts.append(f"возраст от {age_parts[0]} до {age_parts[1]}")
                else:
                    query_parts.append(f"возраст {values[0]}")
            else:
                query_parts.append(values[0])
    ...
    search_type = detect_search_type(search_text)
    if search_type == "unknown":
        search_type = "name"
