...
async def is_subscribed(user_id, context):
    ...
    try:
        # Используем username канала напрямую
...
def format_bigbase_short(data):
    ...
    for record in records[:3]:
        ...
        if base_record:
            first_line = []
            for item in base_record[:2]:
                if isinstance(item, list) and len(item) >= 2:
                    first_line.append(f"{item[0]}: {item[1]}")
            if first_line:
                lines.append(f"├ {' | '.join(first_line)}")
...
def generate_html(data, search_type, query):
    ...
    if nightsearch_data and nightsearch_data.get("status") == "success":
        rows = ""
        for result in nightsearch_data.get("results", [])[:5]:
            fields = result.get("fields", [])
...
async def handle_message(update, context):
    ...
    # Проверяем подписку перед каждым поиском
    if not await is_subscribed(user_id, context):
        await update.message.reply_text(
            "Для использования бота подпишитесь на канал:",
            reply_markup=get_subscribe_keyboard()
