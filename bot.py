...
def format_short(data, query, stats_count, search_type):
    ...
    if jitler_data and isinstance(jitler_data, dict):
        ...
        # Обрабатываем profiles (VK, OK, и т.д.)
                ...
                for platform, info in profiles.items():
                    if info:
                        if platform == "vk" and isinstance(info, dict):
                            ...
                            url = info.get('url', '')
                            if name:
                                # Убираем дату из имени если есть
                                clean_name = re.sub(r'\([^)]*\)', '', name).strip()
                                lines.append(f"├ 🔵 VK: {clean_name}")
                            if url:
                                lines.append(f"├ 🔗 {url}")
                        elif platform == "ok" and isinstance(info, dict):
                            name = info.get('name', '')
                            url = info.get('url', '')
                            if name:
        ...
        # Обрабатываем telegram отдельно
        ...
        for key, label in useful_keys.items():
            if key in jitler_data and jitler_data[key]:
                lines.append(f"├ {label}: {jitler_data[key]}")
        
        # Counts - оставляем статистику
                ...
                for platform, info in v.items():
                    if isinstance(info, dict):
                        ...
                        if name:
                            ...
                            profile_str.append(f"{platform}: {clean_name}")
                        elif url:
