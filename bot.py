...
def format_short(data, query, stats_count, search_type):
    ...
    if jitler_data and isinstance(jitler_data, dict):
        ...
        if "profiles" in jitler_data:
            ...
            if isinstance(profiles, dict):
                for platform, info in profiles.items():
                    if info:
                        if platform == "vk":
                            lines.append(f"├ 🔵 VK: {info.get('name', '')} ({info.get('url', '')})")
                        elif platform == "ok":
                            lines.append(f"├ 🟠 OK: {info}")
        ...
        for key in useful_keys:
            if key in jitler_data and jitler_data[key]:
                emoji_map = {
                    ...
                    'last_name': '👤', 'fio': '👤',
                    'email': '📧', 'address': '📍', 'city': '🏙️',
                    'region': '📍', 'country': '🌍',
                    'birth_date': '🎂', 'birthday': '🎂',
                    'inn': '🆔', 'snils': '🆔'
                }
                emoji = emoji_map.get(key, '•')
                lines.append(f"├ {emoji} {key}: {jitler_data[key]}")
        ...
        if "counts" in jitler_data:
            ...
            if isinstance(counts, dict):
                ...
                if count_str:
                    lines.append(f"├ 📊 Статистика: {count_str}")
    ...
    if dep_data and isinstance(dep_data, dict):
        ...
        if results and len(results) > 0:
            lines.append("")
            lines.append(f"📁 *Найдено в базах Depsearch:* {len(results)} записей")
...
def generate_html(data, search_type, query):
    ...
    if jitler_data and isinstance(jitler_data, dict):
        ...
        for k, v in jitler_data.items():
            ...
            if v is None or v == "":
                continue
            # Красиво форматируем вложенные структуры
            if isinstance(v, (dict, list)):
                v = json.dumps(v, ensure_ascii=False, indent=2)[:500]
            filtered[k] = v
...
async def handle_message(update, context):
    ...
    # Статистика - используем оригинальный текст
