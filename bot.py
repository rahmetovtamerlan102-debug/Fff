...
# ========== КРАТКИЙ ОТЧЁТ ==========
def format_short(data, phone_info, query, stats_count):
    results = data.get("results", [])
    lines = []

    # Ищем ФИО и номер из результатов DepSearch
    fio = None
    phone_from_dep = None
    for r in results[:10]:
        if not fio:
            if "full_name" in r and r["full_name"]:
                fio = r["full_name"]
            elif "fio" in r and r["fio"]:
                fio = r["fio"]
            elif "name" in r and r["name"]:
                fio = r["name"]
        if not phone_from_dep:
            if "phone" in r and r["phone"]:
                phone_from_dep = r["phone"]
            elif "phone2" in r and r["phone2"]:
                phone_from_dep = r["phone2"]
        if fio and phone_from_dep:
            break

    # Если нашли ФИО и номер — используем их для DaData
    if phone_from_dep:
        phone_info = dadata_lookup(phone_from_dep)
        phone_display = fio if fio else phone_from_dep
    else:
        phone_display = query
        if phone_info:
...
async def handle_message(update, context):
    ...
    short = format_short(data, phone_info, text, stats_count)
