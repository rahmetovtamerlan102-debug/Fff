...
def jitler_search_number(phone):
    ...
    try:
        ...
        logging.info(f"Jitler статус: {resp.status_code}")
        
        if resp.status_code == 401:
            return {"error": "Неавторизован (401). Проверьте токен Jitler"}
        elif resp.status_code == 403:
            return {"error": "Доступ запрещён (403)"}
        elif resp.status_code == 429:
            return {"error": "Лимит запросов исчерпан (429)"}
        elif resp.status_code == 501:
            return {"error": "Задача ещё выполняется (501)"}
        elif resp.status_code == 404:
            return {"error": "Данные не найдены (404)"}
        elif resp.status_code != 200:
            return {"error": f"Ошибка Jitler: {resp.status_code}"}
        
        data = resp.json()
        
        if not data.get("result"):
            error_msg = data.get("error", "Неизвестная ошибка")
            return {"error": f"Jitler: {error_msg}"}
        ...
        return {"error": "Jitler: неожиданный формат ответа"}
        
    except requests.Timeout:
        return {"error": "Jitler: таймаут"}
    except requests.ConnectionError:
        return {"error": "Jitler: ошибка соединения"}
    except Exception as e:
        return {"error": f"Jitler: {str(e)}"}
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if resp.status_code == 403:
                return {"error": "Доступ к задаче запрещён"}
            
            return {"error": f"Ошибка при получении результата: {resp.status_code}"}
            
        except Exception:
            time.sleep(1)
            continue
    
    return {"error": "Таймаут ожидания результата"}
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        if jitler_future:
            try:
                ...
                if jitler_result and not isinstance(jitler_result, dict):
                    result["jitler"] = jitler_result
                    logging.info("Jitler данные получены")
                elif isinstance(jitler_result, dict) and "error" in jitler_result:
                    result["errors"].append(f"Jitler: {jitler_result['error']}")
                else:
...
def format_short(data, query, stats_count, search_type):
    ...
    if errors:
        lines.append("")
        lines.append(f"Ошибка: {errors[0]}")
        if len(errors) > 1:
            lines.append(f"Ошибка: {errors[1]}")
