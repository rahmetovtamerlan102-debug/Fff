...
def format_jitler_data(jitler_data):
    ...
    if "phonebooks" in jitler_data:
        ...
        elif isinstance(phonebooks, dict) and phonebooks:
            names = []
            for key, value in phonebooks.items():
                if value:
                    names.append(str(value))
            if names:
                lines.append(f"├ 📇 Телефонные книги: {', '.join(names)}")
    
    # Обрабатываем profiles с универсальной проверкой регистра
    if "profiles" in jitler_data:
        ...
        if isinstance(profiles, dict):
            for platform, info in profiles.items():
                ...
                # VK (любой регистр: vk, VK, Vk и т.д.)
                if platform_name == "vk" and isinstance(info, dict):
                    name = info.get("name", "")
                    url = info.get("url", "")
                    if name:
                ...
                # OK (любой регистр)
                elif platform_name == "ok" and isinstance(info, dict):
                    name = info.get("name", "")
                    url = info.get("url", "")
                    if name:
                ...
                # Telegram (любой регистр)
                elif platform_name == "telegram" and isinstance(info, dict):
                ...
                # Другие соцсети (словарь)
                elif isinstance(info, dict):
                    name = info.get("name") or info.get("username") or info.get("nickname")
                    url = info.get("url")
                    if name:
                    ...
                    if not name and not url:
                        # Если нет name и url, показываем что есть
    ...
    # Обрабатываем telegram отдельно (если есть поле telegram)
...
def generate_html(data, search_type, query):
    ...
    if jitler_data and isinstance(jitler_data, dict):
        ...
        for k, v in jitler_data.items():
            ...
            elif k == 'phonebooks' and isinstance(v, dict):
                names = []
                for key, value in v.items():
                    if value:
                        names.append(str(value))
                clean_data[k] = ', '.join(names) if names else v
            elif k == 'profiles' and isinstance(v, dict):
                profile_str = []
                for platform, info in v.items():
                    if isinstance(info, dict):
                        ...
                        profile_str.append(f"{platform}: {info}")
