...
JITLER_TIMEOUT = 10  # Отдельный таймаут для Jitler
...
def jitler_search_number(phone):
    """Быстрый поиск по номеру через Jitler"""
    ...
    try:
        ...
        # Если создана задача - ждем недолго
        if "id" in data:
        ...
        logging.warning("Jitler timeout")
        return None
    except Exception as e:
...
def jitler_search_id(search_id):
    """Быстрый поиск по ID через Jitler"""
    token = get_jitler_token()
    if not token:
        return None
...
def jitler_wait_for_result_fast(task_id, token, max_attempts=5):
    """Быстрое ожидание результата (максимум 5 попыток)"""
...
def unified_search(query, search_type):
    """Параллельный поиск через Depsearch и Jitler"""
    result = {
        "depsearch": None,
        "jitler": None,
        "phone_info": None,
        ...
        if "profiles" in jitler_data:
            ...
            if isinstance(profiles, dict):
                for platform, info in profiles.items():
                    if info:
                        if platform == "vk" and isinstance(info, dict):
                            name = info.get('name', '')
                            url = info.get('url', '')
                            if name:
                        ...
                        elif platform == "telegram" and isinstance(info, dict):
...
# ========== ГЕНЕРАЦИЯ HTML-ОТЧЁТА (сокращенная версия) ==========
...
async def handle_message(update, context):
    ...
    msg = await update.message.reply_text("⏳ Поиск... (это может занять до 15 секунд)")
