...
# ========== Flask для UptimeRobot и self-ping ==========
...
# ========== SELF-PING (без внешнего URL) ==========
def self_ping():
    url = f"http://localhost:{PORT}/ping"
    while True:
        try:
            requests.get(url, timeout=5)
            logging.info("Self-ping отправлен (localhost)")
...
# ========== КРАТКИЙ ОТЧЁТ (только телефон, оператор, регион, страна) ==========
def format_short(data, query, stats_count):
    ...
    if phone_info:
        ...
        if not region:
            region = phone_info.get("region_with_type")
        country = phone_info.get("country")
        if not country:
            country = phone_info.get("country_iso_code")
...
def main():
    ...
    # Запускаем self-ping (локально, без внешнего URL)
