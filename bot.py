...
if not BOT_TOKEN:
    raise ValueError("❌ BOT_TOKEN не задан! Укажи в .env или в настройках Render.")
if not API_TOKEN:
    raise ValueError("❌ API_TOKEN не задан! Укажи в .env или в настройках Render.")
if not BASE_URL:
    raise ValueError("❌ BASE_URL не задан! Укажи в .env или в настройках Render.")
...
def detect_search_type(text):
    ...
    # Номер карты (16 цифр или с пробелами)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Формируем URL для запроса к DepSearch API
    url = f"{BASE_URL}/quest={q}&token={API_TOKEN}&lang=ru"
    ...
    try:
        ...
        try:
            ...
            await update.message.reply_text(f"❌ API вернул не JSON: {resp.text[:200]}")
            return
        ...
        if not results and not phone_info and not ip_info:
        ...
        # Обрезаем длинный ответ (лимит Telegram — 4096 символов)
        if len(answer) > 4096:
            answer = answer[:4000] + "\n... (обрезано)"
