...
# Канал для подписки (опционально, можно убрать)
...
async def handle_message(update, context):
    ...
    # Если пользователь в режиме поиска по неполным данным
    ...
    # Автоопределение типа для обычного текста
...
async def perform_search(update, context, query_text):
    # Выполняет поиск по произвольному тексту (для неполных данных)
    stats_count = increment_stats(query_text)
    msg = await update.message.reply_text("Поиск...")
    # Определяем тип, но если не определился, используем "name"
    search_type = detect_search_type(query_text)
    if search_type == "unknown":
        search_type = "name"
    data = unified_search(query_text, search_type)
    short = format_short(data, query_text, stats_count, search_type)
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("Полный отчёт", callback_data=f"report_{search_type}_{query_text}")]])
    await msg.edit_text(short, reply_markup=keyboard, parse_mode="Markdown")
    context.user_data['report_data'] = {'search_type': search_type, 'query': query_text, 'data': data}
...
async def partial_search_callback(update, context):
    ...
    await query.edit_message_text(
        "Введите данные для поиска (ФИО, номер телефона, адрес, любая информация).\n\n"
        "Примеры:\n"
        "Иванов Иван\n"
        "Москва, Тверская\n"
        "79999688666",
