...
def format_bigbase_short(data, skip_operator=False):
    lines = []
        ...
        if base_record:
            ...
            for item in base_record[:3]:
                if isinstance(item, list) and len(item) >= 2:
                    ...
                    if skip_operator and key_lower in ["оператор", "регион", "страна", "phone_operator", "phone_region", "phone_country_info"]:
                        continue
...
def format_nightsearch_short(data, personal=None):
    lines = []
    ...
    if personal:
        if personal.get("fio"):
            skip_fields.add("фио")
            skip_fields.add("fio")
            skip_fields.add("full_name")
        if personal.get("birth_date"):
            skip_fields.add("дата рождения")
            skip_fields.add("birth_date")
            skip_fields.add("bdate")
    ...
    for result in results[:10]:
        ...
        for field in fields[:4]:
            ...
            if key in skip_fields:
                continue
            if key in ["телефон", "phone"] and value == data.get("phone"):
                continue
            item = f"{field.get('key')}: {value}" if field.get('key') else value
            if item not in seen:
                lines.append(f"├ {item}")
                seen.add(item)
...
def format_short(data, query, stats_count, search_type):
    ...
    bigbase_data = data.get("bigbase")
    if bigbase_data and isinstance(bigbase_data, dict):
        bb_lines = format_bigbase_short(bigbase_data, skip_operator=True)
        if bb_lines:
            lines.append("")
    ...
    if nightsearch_data and isinstance(nightsearch_data, dict):
        ns_lines = format_nightsearch_short(nightsearch_data, personal)
        if ns_lines:
            lines.append("")
