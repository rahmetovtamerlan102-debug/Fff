...
def increment_stats(query):
    stats = load_stats()
    key = re.sub(r'\D', '', query)  # нормализуем номер
...
def format_short(data, phone_info, query, stats_count):
    ...
    # Если ФИО найдено — подставляем его в телефонную строку
    if fio:
        lines.append("📱")
        lines.append(f"├ Телефон: {fio}")
    else:
        lines.append("📱")
...
async def handle_message(update, context):
    ...
    # Краткий отчёт с реальной статистикой
